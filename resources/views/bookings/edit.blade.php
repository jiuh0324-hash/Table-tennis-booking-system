                    ...
                    <form action="{{ route('bookings.update', $booking) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="table_tennis_table_id" id="table_tennis_table_id" class="form-select" required>
                                ...
                                @foreach($tables as $table)
                                    <option value="{{ $table->id }}" {{ $booking->table_tennis_table_id == $table->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" name="booking_date" id="booking_date" 
                                   class="form-control" value="{{ $booking->booking_date->format('Y-m-d\TH:i') }}" required>
                            @error('booking_date')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="duration_minutes" id="duration_minutes" class="form-select" required>
                                ...
                                <option value="60" {{ $booking->duration_minutes == 60 ? 'selected' : '' }}>60 minutes</option>
                                <option value="90" {{ $booking->duration_minutes == 90 ? 'selected' : '' }}>90 minutes</option>
                                <option value="120" {{ $booking->duration_minutes == 120 ? 'selected' : '' }}>120 minutes</option>
                            </select>
